...
炭膜电阻器 carbon-film resistors和激光器件 Laser 数据集也常应用在Gamma过程中，具体数据在3.3中已给出。
...
li_dat$cell = as.factor(li_dat$cell)
```


```{r}
li_dat %>% ggplot(aes(cycles,ah,color = cell)) +
  geom_line(size = 0.1) + 
  geom_point(size = 0.8) + 
  theme_bw() +
  theme(panel.grid.major = element_blank()) +
...
#### 4.4.5 Device-B data
> 数据来自Meeker, W. Q. (1998). Statistical Methods for Reliability Data

These data came from an accelerated repeated measured degradation test of a
solid-state RF power amplifier and were first used in Meeker, Escobar, and Lu
(1998). 



```{r}
DeviceB_dat = read.csv("dataset/DeviceB.csv")
knitr::kable(head(DeviceB_dat))
DeviceB_dat$Device_Number = as.factor(DeviceB_dat$Device_Number)
```
...
```{r}
DeviceB_dat %>% ggplot(aes(hours,db,color = Device_Number)) +
  geom_line(size = 0.1) + 
  geom_point(size = 0.8) + 
  theme_bw() +
  theme(panel.grid.major = element_blank()) +
